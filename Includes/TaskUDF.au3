...
Func _TaskFolderCreate($folder)
	...
	If IsObj($oFolder) Then $oFolder.CreateFolder($folder)
	If @error Then
		$oService = 0;delete object
		SetError(1)
		Return 0
	Else
		$oService = 0;delete object
...
Func _TaskFolderDelete($folder)
	...
	$oService.Connect()
	$oFolder = $oService.GetFolder("\")
	If IsObj($oFolder) Then $oFolder.DeleteFolder($folder, 0)
	If @error Then
		$oService = 0
		SetError(1)
		Return 0
...
Func _TaskFolderExists($folder = "\")
	...
	If IsObj($oFolder) And Not @error Then
		$oService = 0
		Return 1
	Else
		$oService = 0
...
Func _TaskListAll($folder = "\", $hidden = 1)
	...
	If IsObj($oFolder) Then
		...
		If IsObj($oTasks) And Not @error Then
...
#cs
	...
	$WakeToRun				=> Optional Boolean Default False Wake up the system to run the task
...
#ce
Func _TaskCreate($taskname, $TaskDescription, $TriggerEvent, $StartTrigger, $EndTrigger, $DaysOfWeek, $DaysOfMonth, $MonthOfYear, $WeeksOfMonth, $DaysInterval, $Interval, $RepetitionEnabled, $LogonType, $RunLevel, $username, $passwordTP, $program, $WorkingDirectory = "", $Arguments = "", $RunOnlyIfNetworkAvailable = True, $active = True, $multiinst = 0, $nobatstart = False, $stoponBat = False, $hidden = False, $idle = False, $WakeToRun = False, $timelimit = "P0D", $priority = 5, $Duration = "", $StartWhenAvailable = True)
	If Not _TaskIsValidPlatfrom() Then Return SetError(2, 2, 0)
	...
	$oFolder.RegisterTaskDefinition($taskname, $oTaskDefinition, 6, $username, $passwordTP, $LogonType, "")
...
Func _TaskIsValidPlatfrom()
	If Not @error And @OSVersion = "WIN_XP" Or @OSVersion = "WIN_XPe" Or @OSVersion = "WIN_2000" Or @OSVersion = "WIN_2003" Then ;win Vista or newer and Autoit 3.3.9.4 or newer needed for this uDF to work
		Return 0
	Else
		Return 1
	EndIf
